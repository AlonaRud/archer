...
namespace CBS.UI
{
    public class EquipmentInventory : MonoBehaviour
    {
        ...
        private string CurrentCategory { get; set; } = "Equipment"; // Группа Equipment
        ...
        private void DisplayCategories()
        {
            ...
            CurrentCategories = new[] { "Equipment" }; // Только Equipment
        ...
        private void OnCategorySelected(string category)
        {
            ...
            ItemsScroller.Clear();
            CBSInventory.GetInventoryByCategory("Equipment", OnGetInventory);
        }

        private void OnGetInventory(CBSGetInventoryResult result)
{
    Debug.Log($"Equipment inventory result: IsSuccess={result.IsSuccess}, ItemsCount={(result.NonEquippedItems?.Count ?? 0)}");
    if (result.IsSuccess)
    {
        CurrentItems = result.NonEquippedItems;
        Debug.Log($"Equipment items: {string.Join(", ", CurrentItems.Select(i => $"{i.ItemID} (Category: {i.Category})"))}");
        DrawItems();
    }
    else
    {
        Debug.LogError($"Failed to load Equipment: {result.Error?.ToString()}");
    }
}
