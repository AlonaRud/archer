...
namespace CBS
{
    public class CBSFriendsModule : CBSModule, IFriends
    {
        ...
        /// <summary>
        /// Get a list of users to whom you have sent a friend request
        /// </summary>
        /// <param name="profileConstraints"></param>
        /// <param name="result"></param>
        public void GetOutgoingRequests(CBSProfileConstraints profileConstraints, Action<CBSGetFriendsResult> result)
        {
            var profileID = Profile.ProfileID;
            var constraints = profileConstraints ?? GetDefaultProfileConstraints();
            FabFriends.GetOutgoingRequests(profileID, constraints, onGet =>
            {
                var cbsError = onGet.GetCBSError();
                if (cbsError != null)
                {
